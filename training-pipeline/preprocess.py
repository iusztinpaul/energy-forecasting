...
def split_data(
    ...
    """
    Template for splitting data into train, validation and test sets.
    ...
    min_validation_split_datetime = max_datetime - pd.DateOffset(
        days=test_split_size_days + validation_split_size_days
    )

    train_mask = df["datetime_utc"] < min_validation_split_datetime
    validation_mask = df["datetime_utc"].between(
        min_validation_split_datetime, min_test_split_datetime, inclusive="left"
    )
...
def encode_categorical(
    ...
    consumer_type_hashing_encoder = hashing.HashingEncoder(
        return_df=True, cols=["consumer_type"]
    )
    ...
    area_one_hot_encoder = one_hot.OneHotEncoder(
        return_df=True, cols=["area"], handle_unknown="error"
    )
